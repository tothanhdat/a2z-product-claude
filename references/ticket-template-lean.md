...
**Context:**
[Technical approach — API endpoints, call sequence, key differences from similar actions. Max 5 câu.]
...
- Goal = business language; Context = technical language.
- Context phải name exact API endpoints (method + path).
- Nếu action tương tự action khác, explicitly state difference.
...
**AC.04. Action * displays "[Action Name]" as pre-selected value**
...
Then the action surfaces: "[Service] rate limit reached. Retrying with exponential backoff (max 3 attempts).",
And if all retries fail, the workflow execution is marked as failed

**Edge Cases:**
- Service account email included in error detail
```
...
- Story [N] (Error Handling) is a cross-cutting concern that applies to all other stories.

**External Dependencies:**
- **[Service] API** — [endpoint 1] ([method]), [endpoint 2] ([method])
- **OAuth 2.0** — requires scope: [scope string]
- **Connected Account** — must have [minimum permission level] on [resource]
```
...
- Related operations in other nodes
- API capabilities not exposed in the UI
- Batch / bulk operations
- Reverse / undo operations
- Edge cases deferred to a later phase
...
- Chỉ giữ edge case khi ảnh hưởng trực tiếp đến user-facing behavior, QA coverage, data correctness, security, hoặc scope.
...
- Technical field names, API endpoints: English.
...
### Config-time / Runtime Error Message Consistency
- Với validation đơn giản (empty, length exceeded, invalid characters): **runtime message phải GIỐNG config-time message**. Không dùng message generic khi có thể trả message tường minh.
- Chỉ dùng message khác khi runtime detect lỗi mà **config-time không thể validate** (VD: URL trỏ tới resource không tồn tại, format phức tạp chỉ provider mới biết).
- **Ví dụ ĐÚNG:** Config `"Text must be 4,096 characters or less."` → Runtime cùng case: `"Text must be 4,096 characters or less."`
- **Ví dụ SAI:** Config `"Text is required."` → Runtime: ~~`"Text resolved to an empty value. Check your variable mapping."`~~ (cùng case empty nhưng message khác = KHÔNG chấp nhận)
