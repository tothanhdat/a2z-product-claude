...
- **Không mô tả placeholder text cho bất kỳ field nào** — placeholder hiển thị trên Figma design, dev follow từ Figma. Ticket không cần nhắc.
...
- **KHÔNG liệt kê lại options** ở Business Rules hoặc bất kỳ section nào khác.
...
Tooltips section **chỉ được thêm vào ticket khi user khai báo rõ từ đầu** rằng một hoặc nhiều field cần tooltip. Nếu user không đề cập, **không tự thêm section này**.

**Vị trí:** Cuối ticket, sau Error Handling.
...
| Field | Tooltip Text |
|---|---|
| [Field Name] | [Nội dung tooltip — mô tả ngắn, dễ hiểu cho business user] |
```

**Rules:**
- Chỉ liệt kê các field được user khai báo cần tooltip — không tự suy diễn.
- Tooltip text viết cho business user, không technical.
- Nếu tooltip text chưa được user cung cấp, hỏi trước khi điền.
- Thứ tự sections cuối ticket: ... → Business Rules → Error Handling → **Tooltips** (nếu có).
